      ...
      <div style={{ maxWidth: 420, margin: '80px auto', padding: '0 20px' }}>
        <div className="card" style={{ textAlign: 'center' }}>
          ...
          <div className="form-group" style={{ textAlign: 'left' }}>
            <label>아이디</label>
            <input placeholder="아이디 입력" value={id} onChange={e => setId(e.target.value)} onKeyUp={e => e.key === 'Enter' && submit()} />
          </div>
          <div className="form-group" style={{ textAlign: 'left' }}>
            <label>비밀번호</label>
            <input type="password" placeholder="비밀번호 입력" value={pw} onChange={e => setPw(e.target.value)} onKeyUp={e => e.key === 'Enter' && submit()} />
          </div>

          {error && <p className="error-msg" style={{ marginBottom: 10 }}>{error}</p>}

          <button className="btn-primary" onClick={submit} style={{ marginTop: 6 }} disabled={loading}>
            {loading ? '로그인 중...' : '로그인'}
          </button>
