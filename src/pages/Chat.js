...
export default function Chat() {
  ...
  const [room, setRoom] = useState(searchParams.get('room') || 'global');
        ...
        <div className="chat-layout">
            ...
            <div className="chat-rooms">
              {ROOMS.map(r => {
                ...
                return (
                  <div key={r.id}
                    ...
                    onClick={() => { if (!locked) changeRoom(r.id); }}
              ...
              {dmRooms.length > 0 && (
                <>
                  ...
                  {dmRooms.map(dmKey => {
                    const otherId = dmKey.replace('dm_','').replace(user.id,'').replace(/_/g,'');
                    const other = getMember(otherId);
                    return (
                      <div key={dmKey} className={`chat-room-item ${room===dmKey?'active':''}`} onClick={()=>changeRoom(dmKey)}>
                        <div className="chat-room-icon">{other?.gender==='남'?'👦':'👧'}</div>
                        <div>
                          <div className="chat-room-name">@{otherId}</div>
                          <div className="chat-room-desc">{other?.name_public ? other?.name : other?.name?.charAt(0)+'**'}</div>
                        </div>
                      </div>
                    );
                  })}
          ...
          <div className="chat-main">
            <div className="chat-header">
              {currentRoom?.icon || '💌'} {currentRoom?.name || '1:1 채팅'}
