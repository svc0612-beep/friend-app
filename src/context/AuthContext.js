...
export function AuthProvider({ children }) {
      ...
      const formatted = await Promise.all(data.map(async m => {
        ...
        return {
          ...
          messages: (msgs || []).map(msg => ({
            id: msg.id, from: msg.from_id, body: msg.body,
    ...
    if (data) {
      ...
      data.forEach(msg => {
        ...
        grouped[msg.room_id].push({
          id: msg.id, from: msg.from_id, text: msg.text,
          time: new Date(msg.created_at).toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit' })
        });
      });
  ...
  const register = async (data) => {
    ...
    const { error } = await supabase.from('members').insert([{
      ...
      age: data.age, gender: data.gender, dept: data.dept, mbti: data.mbti,
  ...
  const login = async (id, password) => {
    ...
    if (data?.length > 0) {
      const found = members.find(m => m.id === id) || { ...data[0], namePublic: data[0].name_public, interests: [], messages: [], likedBy: [], blockedIds: [] };
      setUser(found);
      return { ok: true };
    }
  ...
  const recordVisit = async (targetId) => {
    ...
    await supabase.from('members').update({ visitors: (target?.visitors || 0) + 1 }).eq('id', targetId);
    await loadMembers();
  };
  ...
  const updateProfile = async (data) => {
    await supabase.from('members').update({
      name: data.name, name_public: data.namePublic, age: data.age,
      gender: data.gender, dept: data.dept, mbti: data.mbti,
      intro: data.intro, phone: data.phone, email: data.email,
      drama: data.drama, movie: data.movie, music: data.music,
      interest: data.interest, ideal: data.ideal
    }).eq('id', user.id);
    await loadMembers();
  };
